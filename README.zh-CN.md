...
> ⚠️ 个人自用工具，非 Anthropic 官方产品。支持 **Terminal.app**、**iTerm2** 与 **Otty**（可在设置中选择目标或自动检测；Otty 需 1.2.0+）。
...
- **拖文件进编辑器**：插入文件路径
- **双击 Option 搜索笔记**（Obsidian 等 Markdown 库）：置顶浮层，Markdown 预览可框选复制并定位到匹配处；库路径与排除目录可在设置中配置
...
- **常用语**：预设常用文本片段，菜单栏点击插入，或按 ⌃1–⌃9 快捷键。可在设置中编辑（每行一条）
- **一键发送**：⌘↵ → 若有多个终端窗口/标签，弹菜单按标题选择 → 内容粘进 Claude 并回车、编辑器清空
- **Terminal.app、iTerm2 与 Otty**：均可发送（或自动检测）；在设置中选择目标终端
- **历史记录**：发送过的内容存进菜单，点击可再次填入编辑器（保留条数可设）
- **可自定义**：三个双击手势的修饰键、笔记库路径 / 排除目录、目标终端、历史条数、菜单标题宽度、⌘⇧4 截图保存路径（写入系统设置，重启后仍生效；菜单栏 → 设置…）
...
| 双击 **Option** | 搜索笔记（文件名 + 正文）；Markdown 预览并定位到匹配处 |
| 双击 **Command** | 用 Sublime Text 打开 Finder 当前选中的文件 |
| **按住右 Option** | 语音录入（实时识别，松手停止）；底部栏红点脉冲指示 |
| **⌃1–⌃9** | 插入对应的常用语到光标位置 |
| 拖文件到编辑器 | 插入其路径 |
| ⌘⇧4 截图 | 新截图路径插入编辑器 |
| **⌘↵** | 发送到终端（多个窗口/标签则弹选择菜单） |
...
- **自动化**：控制 Finder、Terminal.app、iTerm2 和/或 Otty（枚举窗口/标签、发送内容）
- **语音识别**：macOS 原生 SFSpeechRecognizer，实时转写口述内容
- **麦克风**：仅在按住语音键录音时使用
- **截图保存目录**：监听系统截图路径（默认为桌面；可在设置中更改）
